...
namespace UnityEngine.Experimental.Rendering.ModularSRP
{
    ...
    public class SetupLightweightConstantsPass : ScriptableRenderPass
    {
        [RenderPassInput("PerObjectLightIndices")]
        RenderPassReference<ComputeBuffer> m_PerObjectLightIndices;
        [RenderPassInput("MaxVisibleAdditionalLights")]
        RenderPassReference<int> m_MaxVisibleAdditionalLights;
        [RenderPassInput("RenderingData")]
        RenderPassReference<RenderingData> m_RenderingData; // Input automagically assigned from a previous pass
        ...
        /// Create the pass
        /// </summary>
        public SetupLightweightConstantsPass()
        {
        ...
        void SetupShaderLightConstants(CommandBuffer cmd, ref LightData lightData)
        {
            ...
            for (int i = 0; i < m_MaxVisibleAdditionalLights.Value; ++i)
        ...
        void SetupAdditionalLightConstants(CommandBuffer cmd, ref LightData lightData)
        {
            ...
            if (lightData.additionalLightsCount > 0)
            {
                ...
                for (int i = 0; i < lights.Count && additionalLightsCount < m_MaxVisibleAdditionalLights.Value; ++i)
                {
                ...
                if (m_PerObjectLightIndices.Value != null)
                    cmd.SetGlobalBuffer(LightConstantBuffer._AdditionalLightsBuffer, m_PerObjectLightIndices.Value);
            }
            else
            {
        ...
        {
            int maxVisibleAdditionalLights = m_MaxVisibleAdditionalLights.Value;
            ...
            SetupShaderLightConstants(cmd, ref m_RenderingData.Value.lightData);
            SetShaderKeywords(cmd, ref m_RenderingData.Value.cameraData, ref m_RenderingData.Value.lightData, ref m_RenderingData.Value.shadowData);
